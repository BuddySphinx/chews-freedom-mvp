  ...
  for (const target of eligibleRescueTargets(state)) {
    for (let actorIndex = 0; actorIndex < 3; actorIndex += 1) {
      for (let patientIndex = 0; patientIndex < 3; patientIndex += 1) {
        // A helpful rescue lowers the chosen Tyrosinemia Kid's total. It does not have
...
function chooseAiPatientCard(state: GameState, aiSeat: Seat, humanSeat: Seat, humanIndex: number): { firstIndex: number; secondIndex: number } {
  ...
  if (!Number.isInteger(humanIndex) || humanIndex < 0 || humanIndex >= 3) throw new Error("Choose one of the human Tyrosinemia Kid's cards.");
...
function startRound(state: GameState): void {
  ...
  message(state, "ACTIVE_RESCUE", "ACTIVE_RESCUE_CHOICE", failing.length ? "Active nutritionist may choose either Tyrosinemia Kid who is over the protein limit." : "Both Tyrosinemia Kids are within the protein limit. Rescue is skipped.");
}
...
function beginVegetableResolution(state: GameState): void {
  ...
  if (state.gardenTokens <= 0) {
    message(state, "VEGETABLE_RESOLUTION", "GARDEN_EXHAUSTED", "No zero-protein cabbages remain while a Tyrosinemia Kid is still over the protein limit.");
    settleRound(state);
    return;
  }
  state.phase = "VEGETABLE_RESOLUTION";
  message(state, "VEGETABLE_RESOLUTION", "GARDEN_PHASE", "Garden phase: choose the highest-protein food card on the highlighted Tyrosinemia Kid to replace with a zero-protein cabbage.");
}
...
function executeVegetableReplacement(state: GameState, patient: Seat, cardIndex: number, automated: boolean): void {
  ...
  if (!action) throw new Error("Choose a highest-protein food card on the highlighted Tyrosinemia Kid for the cabbage replacement.");
  replaceWithVegetable(state, action.patient, action.cardIndex);
  if (!failingPatients(state).length) {
    message(state, "VEGETABLE_RESOLUTION", automated ? "AI_GARDEN_COMPLETE" : "GARDEN_COMPLETE", "Every Tyrosinemia Kid is now within the protein limit.");
    settleRound(state);
    return;
  }
  if (state.gardenTokens === 0) {
    message(state, "VEGETABLE_RESOLUTION", "GARDEN_EXHAUSTED", "The field used every available zero-protein cabbage, but a Tyrosinemia Kid is still over the protein limit.");
    settleRound(state);
    return;
  }
  message(state, "VEGETABLE_RESOLUTION", automated ? "AI_GARDEN_REPLACEMENT" : "GARDEN_REPLACEMENT", "A cabbage was taken. Choose the next highlighted highest-protein card while the Tyrosinemia Kid remains over the protein limit.");
}
...
function settleRound(state: GameState): void {
  ...
  const outcome: RoundOutcome = unresolvedPatients
    ? { round: state.round, kind: "UNRESOLVED", title: "The garden ran out.", detail: "At least one Tyrosinemia Kid remained over the protein limit." }
    : state.phase === "VEGETABLE_RESOLUTION"
      ...
      : state.phase === "PATIENT_SWAP"
        ? { round: state.round, kind: "PATIENT", title: "We succeeded together.", detail: "Tyrosinemia Kid mutual aid completed the rescue." }
        : { round: state.round, kind: "NUTRITIONIST", title: "The nutritionists were amazing.", detail: "The rescue was completed before Tyrosinemia Kid aid was needed." };
  ...
  if (unresolvedPatients || state.gardenTokens === 0) {
    ...
    state.endReason = unresolvedPatients ? "Garden supply exhausted before every Tyrosinemia Kid was within the protein limit" : "Garden supply exhausted";
...
function advanceAfterRescue(state: GameState): void {
  if (state.phase === "ACTIVE_RESCUE") {
    if (!failingPatients(state).length) {
      message(state, "ACTIVE_RESCUE", "ACTIVE_RESCUE_COMPLETE", "The active nutritionist brought every Tyrosinemia Kid within the protein limit. Assistant rescue and Tyrosinemia Kid mutual aid are skipped.");
      settleRound(state);
      return;
    }
    state.phase = "ASSISTANT_RESCUE";
    const target = state.rescueTarget;
    message(state, "ASSISTANT_RESCUE", "SUPPORT_CHOICE", target === null ? "The active nutritionist did not make a helpful swap. At least one Tyrosinemia Kid is still over the protein limit, so the assistant gets one rescue attempt and may choose either Tyrosinemia Kid who needs help." : `Active nutritionist chose Tyrosinemia Kid Seat ${target + 1}, but at least one Tyrosinemia Kid is still over the protein limit. The assistant now gets one rescue attempt and may choose either Tyrosinemia Kid who needs help.`);
  } else if (state.phase === "ASSISTANT_RESCUE") {
    if (!failingPatients(state).length) settleRound(state);
    else {
      state.phase = "PATIENT_SWAP";
      message(state, "PATIENT_SWAP", "PATIENT_PHASE", "At least one Tyrosinemia Kid is still over the protein limit. The Tyrosinemia Kids may swap once or pass.");
    }
  }
}
  ...
  while (state.phase !== "GAME_OVER" && safety-- > 0) {
    if (state.phase === "ACTIVE_RESCUE") {
      ...
      if (!legalRescues(state, actor).length) {
        message(state, "ACTIVE_RESCUE", "RESCUE_SKIPPED_NO_HELPFUL_ACTION", "Active nutritionist has no helpful swap for either over-limit Tyrosinemia Kid. The assistant receives the next rescue attempt.");
    ...
    if (state.phase === "ASSISTANT_RESCUE") {
      ...
      if (!legalRescues(state, actor).length) {
        message(state, "ASSISTANT_RESCUE", "RESCUE_SKIPPED_NO_HELPFUL_ACTION", "Assistant has no helpful swap for either over-limit Tyrosinemia Kid. Tyrosinemia Kid swap and garden resolution remain available.");
        advanceAfterRescue(state);
        continue;
      }
      if (state.controllers[actor] !== "AI") return;
    ...
    if (state.phase === "PATIENT_SWAP") {
      ...
      else { message(state, "PATIENT_SWAP", "AI_PASS", "Tyrosinemia Kids chose not to swap."); beginVegetableResolution(state); }
      continue;
    }
...
function executeRescue(state: GameState, actor: Seat, actorIndex: number, target: Seat, patientIndex: number, automated: boolean): void {
  ...
  if (!action) throw new Error("That swap is not a helpful rescue for the selected Tyrosinemia Kid.");
  ...
  message(
    ...
    completedRescue
      ? `Seat ${actor + 1} rescued Tyrosinemia Kid Seat ${action.target + 1} and earned 1 nutritionist point.`
      : `Seat ${actor + 1} made a helpful swap for Tyrosinemia Kid Seat ${action.target + 1}. The Tyrosinemia Kid is still over the protein limit, so the next recovery step will be tried.`
  );
  advanceAfterRescue(state);
}

function executePatientSwap(state: GameState, firstIndex: number, secondIndex: number, automated: boolean): void {
  if (state.phase !== "PATIENT_SWAP") throw new Error("Tyrosinemia Kid swapping is not available now.");
  if (![firstIndex, secondIndex].every((index) => Number.isInteger(index) && index >= 0 && index < 3)) throw new Error("Choose one card from each Tyrosinemia Kid.");
  const { patient1, patient2 } = state.currentRoles;
  ...
  message(state, "PATIENT_SWAP", automated ? "AI_PATIENT_SWAP" : "PATIENT_SWAP", `Tyrosinemia Kids swapped once. Each Tyrosinemia Kid earned ${points} mutual-aid point${points === 1 ? "" : "s"}.`);
...
export function command(state: GameState, input: { type: "RESCUE" | "RESCUE_PASS" | "PATIENT_SWAP" | "PATIENT_PASS" | "TAKE_VEGETABLE" | "ADVANCE_AI"; actor?: number; actorIndex?: number; target?: number; targetIndex?: number; patient1Index?: number; patient2Index?: number; patient?: number; cardIndex?: number; expectedRevision: number }): GameState {
  ...
  if (input.type === "ADVANCE_AI") {
    ...
    if (patient1Ai && patient2Ai) {
      const action = choosePatientSwap(state);
      if (action && action.firstIndex >= 0) executePatientSwap(state, action.firstIndex, action.secondIndex, true);
      else { message(state, "PATIENT_SWAP", "AI_PASS", "AI Tyrosinemia Kids chose not to swap."); beginVegetableResolution(state); }
      ...
      message(state, "PATIENT_SWAP", "AI_PATIENT_CHOICE", `AI Seat ${aiSeat + 1} chose its best card to support Tyrosinemia Kid Seat ${humanSeat + 1}.`);
    ...
    if (state.phase !== "PATIENT_SWAP") throw new Error("A Tyrosinemia Kid pass is not available now.");
    message(state, "PATIENT_SWAP", "PATIENT_PASS", "Tyrosinemia Kids chose not to swap this round.");
    beginVegetableResolution(state);
  } else {
