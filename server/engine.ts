  ...
  for (const target of eligibleRescueTargets(state)) {
    for (let actorIndex = 0; actorIndex < 3; actorIndex += 1) {
      for (let patientIndex = 0; patientIndex < 3; patientIndex += 1) {
        // A helpful rescue lowers the chosen Garden Friend's total. It does not have
...
function chooseAiPatientCard(state: GameState, aiSeat: Seat, humanSeat: Seat, humanIndex: number): { firstIndex: number; secondIndex: number } {
  ...
  if (!Number.isInteger(humanIndex) || humanIndex < 0 || humanIndex >= 3) throw new Error("Choose one of the human Garden Friend’s cards.");
...
function startRound(state: GameState): void {
  ...
  message(state, "ROUND_START", "ROUND_STARTED", `Round ${state.round} begins. Today’s Nutritionist is Seat ${state.currentRoles.active + 1}.`);
  const failing = failingPatients(state);
  message(state, "ACTIVE_RESCUE", "ACTIVE_RESCUE_CHOICE", failing.length ? "Today’s Nutritionist may choose either Garden Friend who is over the protein limit." : "Both Garden Friends are within the protein limit. Rescue is skipped.");
}
...
function replaceWithVegetable(state: GameState, seat: Seat, index: number): void {
  ...
  message(state, "VEGETABLE_RESOLUTION", "VEGETABLE_REPLACEMENT", `A zero-protein Orchard fruit replaced ${displaced.value} protein for Seat ${seat + 1}.`);
}
...
function beginVegetableResolution(state: GameState): void {
  ...
  if (state.gardenTokens <= 0) {
    message(state, "VEGETABLE_RESOLUTION", "GARDEN_EXHAUSTED", "No zero-protein Orchard fruit remains while a Garden Friend is still over the protein limit.");
    settleRound(state);
    return;
  }
  state.phase = "VEGETABLE_RESOLUTION";
  message(state, "VEGETABLE_RESOLUTION", "GARDEN_PHASE", "Orchard phase: choose the highest-protein food card on the highlighted Garden Friend to replace with a zero-protein Orchard fruit.");
}
...
function executeVegetableReplacement(state: GameState, patient: Seat, cardIndex: number, automated: boolean): void {
  ...
  if (!action) throw new Error("Choose a highest-protein food card on the highlighted Garden Friend for the Orchard fruit replacement.");
  replaceWithVegetable(state, action.patient, action.cardIndex);
  if (!failingPatients(state).length) {
    message(state, "VEGETABLE_RESOLUTION", automated ? "AI_GARDEN_COMPLETE" : "GARDEN_COMPLETE", "Every Garden Friend is now within the protein limit.");
    settleRound(state);
    return;
  }
  if (state.gardenTokens === 0) {
    message(state, "VEGETABLE_RESOLUTION", "GARDEN_EXHAUSTED", "The Orchard used every available zero-protein fruit, but a Garden Friend is still over the protein limit.");
    settleRound(state);
    return;
  }
  message(state, "VEGETABLE_RESOLUTION", automated ? "AI_GARDEN_REPLACEMENT" : "GARDEN_REPLACEMENT", "An Orchard fruit was picked. Choose the next highlighted highest-protein card while the Garden Friend remains over the protein limit.");
}
...
function settleRound(state: GameState): void {
  ...
  const outcome: RoundOutcome = unresolvedPatients
    ? { round: state.round, kind: "UNRESOLVED", title: "The Orchard ran out.", detail: "At least one Garden Friend remained over the protein limit." }
    : state.phase === "VEGETABLE_RESOLUTION"
      ? { round: state.round, kind: "GARDEN", title: "The Orchard came through.", detail: "A zero-protein Orchard fruit completed the rescue." }
      : state.phase === "PATIENT_SWAP"
        ? { round: state.round, kind: "PATIENT", title: "We succeeded together.", detail: "Garden Friend mutual aid completed the rescue." }
        : { round: state.round, kind: "NUTRITIONIST", title: "The nutritionists were amazing.", detail: "The rescue was completed before Garden Friend aid was needed." };
  ...
  if (unresolvedPatients || state.gardenTokens === 0) {
    ...
    state.endReason = unresolvedPatients ? "Orchard fruit ran out before every Garden Friend was within the protein limit" : "Orchard fruit ran out";
...
function advanceAfterRescue(state: GameState): void {
  if (state.phase === "ACTIVE_RESCUE") {
    if (!failingPatients(state).length) {
      message(state, "ACTIVE_RESCUE", "ACTIVE_RESCUE_COMPLETE", "Today’s Nutritionist brought every Garden Friend within the protein limit. Assistant rescue and Garden Friend mutual aid are skipped.");
      settleRound(state);
      return;
    }
    state.phase = "ASSISTANT_RESCUE";
    const target = state.rescueTarget;
    message(state, "ASSISTANT_RESCUE", "SUPPORT_CHOICE", target === null ? "Today’s Nutritionist did not make a helpful swap. At least one Garden Friend is still over the protein limit, so the Assistant gets one rescue attempt and may choose either Garden Friend who needs help." : `Today’s Nutritionist chose Garden Friend Seat ${target + 1}, but at least one Garden Friend is still over the protein limit. The Assistant now gets one rescue attempt and may choose either Garden Friend who needs help.`);
  } else if (state.phase === "ASSISTANT_RESCUE") {
    if (!failingPatients(state).length) settleRound(state);
    else {
      state.phase = "PATIENT_SWAP";
      message(state, "PATIENT_SWAP", "PATIENT_PHASE", "At least one Garden Friend is still over the protein limit. The Garden Friends may swap once or pass.");
    }
  }
}
  ...
  while (state.phase !== "GAME_OVER" && safety-- > 0) {
    if (state.phase === "ACTIVE_RESCUE") {
      ...
      if (!legalRescues(state, actor).length) {
        message(state, "ACTIVE_RESCUE", "RESCUE_SKIPPED_NO_HELPFUL_ACTION", "Today’s Nutritionist has no helpful swap for either over-limit Garden Friend. The Assistant receives the next rescue attempt.");
    ...
    if (state.phase === "ASSISTANT_RESCUE") {
      ...
      if (!legalRescues(state, actor).length) {
        message(state, "ASSISTANT_RESCUE", "RESCUE_SKIPPED_NO_HELPFUL_ACTION", "Assistant has no helpful swap for either over-limit Garden Friend. Garden Friend swap and Orchard resolution remain available.");
        advanceAfterRescue(state);
        continue;
      }
      if (state.controllers[actor] !== "AI") return;
    ...
    if (state.phase === "PATIENT_SWAP") {
      ...
      else { message(state, "PATIENT_SWAP", "AI_PASS", "Garden Friends chose not to swap."); beginVegetableResolution(state); }
      continue;
    }
...
function executeRescue(state: GameState, actor: Seat, actorIndex: number, target: Seat, patientIndex: number, automated: boolean): void {
  ...
  if (!action) throw new Error("That swap is not a helpful rescue for the selected Garden Friend.");
  ...
  message(
    ...
    completedRescue
      ? `Seat ${actor + 1} rescued Garden Friend Seat ${action.target + 1} and earned 1 Today’s Nutritionist point.`
      : `Seat ${actor + 1} made a helpful swap for Garden Friend Seat ${action.target + 1}. The Garden Friend is still over the protein limit, so the next recovery step will be tried.`
  );
  advanceAfterRescue(state);
}

function executePatientSwap(state: GameState, firstIndex: number, secondIndex: number, automated: boolean): void {
  if (state.phase !== "PATIENT_SWAP") throw new Error("Garden Friend swapping is not available now.");
  if (![firstIndex, secondIndex].every((index) => Number.isInteger(index) && index >= 0 && index < 3)) throw new Error("Choose one card from each Garden Friend.");
  const { patient1, patient2 } = state.currentRoles;
  ...
  message(state, "PATIENT_SWAP", automated ? "AI_PATIENT_SWAP" : "PATIENT_SWAP", `Garden Friends swapped once. Each Garden Friend earned ${points} mutual-aid point${points === 1 ? "" : "s"}.`);
...
export function command(state: GameState, input: { type: "RESCUE" | "RESCUE_PASS" | "PATIENT_SWAP" | "PATIENT_PASS" | "TAKE_VEGETABLE" | "ADVANCE_AI"; actor?: number; actorIndex?: number; target?: number; targetIndex?: number; patient1Index?: number; patient2Index?: number; patient?: number; cardIndex?: number; expectedRevision: number }): GameState {
  ...
  if (input.type === "ADVANCE_AI") {
    ...
    if (patient1Ai && patient2Ai) {
      const action = choosePatientSwap(state);
      if (action && action.firstIndex >= 0) executePatientSwap(state, action.firstIndex, action.secondIndex, true);
      else { message(state, "PATIENT_SWAP", "AI_PASS", "AI Garden Friends chose not to swap."); beginVegetableResolution(state); }
      ...
      message(state, "PATIENT_SWAP", "AI_PATIENT_CHOICE", `AI Seat ${aiSeat + 1} chose its best card to support Garden Friend Seat ${humanSeat + 1}.`);
    ...
    if (state.phase !== "PATIENT_SWAP") throw new Error("A Garden Friend pass is not available now.");
    message(state, "PATIENT_SWAP", "PATIENT_PASS", "Garden Friends chose not to swap this round.");
    beginVegetableResolution(state);
  } else {
